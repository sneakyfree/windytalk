...
- **Brain → Windy Mind.** Every LLM turn goes through **`POST api.windymind.ai/v1/chat`** (OpenAI-compatible request/response *shape*; SSE streaming via `stream:true`). Never a provider SDK directly. Mind *is* the BYOM/Switzerland layer. ⚠ **Verified live 2026-07-08: the route is `/v1/chat` — `/v1/chat/completions` 404s.** (Earlier drafts of this plan and ADR-058 had the wrong path.) A Mind-side alias `/v1/chat/completions → /v1/chat` is proposed so off-the-shelf OpenAI SDKs point at Mind unchanged; until it's deployed, call the raw route with plain HTTP/SSE, not an OpenAI SDK.
